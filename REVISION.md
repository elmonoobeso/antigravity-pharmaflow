...
- [x] Lote 3c ml/engine.py GRUPOS_FEATURES (Historia de ventas obligatoria, Calendario, Clima, Grupo terapeutico, Promociones; fuera Epi/Perfil por ser constantes) + _seleccionar_grupos (eliminacion hacia atras con validacion temporal solo en train; si quitar varios a la vez empeora, se mantienen todas); prediccion usa feature_names_in_ del modelo. Laboratorio ML: grafico de seleccion + tabla, linea temporal con fechas (ventas reales, zonas train/holdout, ML y baseline en backtest), grafico walk-forward por paso. Verificado AppTest entrenar + Lab sin excepciones; Farmacia_Test descarta Clima (-0,57%), RMSE validacion 4,651->4,624. Observacion para 3b: mejora vs baseline oscila entre ejecuciones (-2,7% / -13,2%) con solo 3 meses de backtest -> metricas con mucha varianza.
- [x] Lote 3b tests/test_ml_engine.py (py -3.14 tests/test_ml_engine.py, datos sinteticos, no toca farmacias/): 6/6 pasan -> sin fugas en las 13 variables, patron conocido gana al baseline (+36%), ruido puro y ventas barajadas no bajan del minimo posible, prediccion futura cubre los meses sin NaN, backtest a varios meses no depende de ventas posteriores al corte. Anadido _backtest_horizonte (holdout encadenado como el pedido) en entrenamiento, pipeline y Laboratorio ML (grafico por mes + KPIs). AppTest OK. Farmacia_Test a 3 meses: ML -22,6% vs baseline.
- HALLAZGO DISENO (de las pruebas, pendiente de aprobacion): el ML queda lejos del optimo con patron claro (22,4 vs minimo 9,7) y sobreajusta con ruido (11,8 vs 8,0; peor que la media con barajadas). Causa: XGBoost predice unidades absolutas y no extrapola tendencias fuera del rango visto; faltan variables de nivel. Experimento (scratch): Media_12m + Mirroring_Ajustado -> ruido 9,4, patron 21,3, horizonte 3m +16%->+25,5%, sin fugas. Propuesta: (a) anadir esas variables, (b) objetivo relativo (ventas / Media_12m) y reescalar, (c) mas regularizacion en la rejilla.
- [x] Lote 3d ml/engine.py VERSION_MOTOR=2: variables de nivel (Media_12m, Mirroring_Ajustado, Mirroring_Rel, Lag_Rel); objetivo relativo ventas/max(Media_12m,1) con peso por nivel (_entrenar/_predecir en todos los fit/predict); rejilla con 3 combinaciones regularizadas; modelos de otra version = obsoletos. Test nuevo de estacionalidad. Banco 7/7: ruido 11,83->9,09 (min 7,97); barajadas 64,9->52,9 (= media, sin sobreajuste); horizonte 3m mejora 16%->39,4%, error por mes +1 20,5->13,2 / +2 39,2->32,1 / +3 30,0->22,1; estacionalidad may-jul vs feb-abr x1,70 (real x1,73), total verano 8% error. Empeora 1 mes con patron claro 22,4->26,4 (sigue ganando al baseline 35,2). Farmacia_Test: 1 mes -13,2%->+11,6%, 3 meses -22,6%->-8,3%. AppTest entrenar + pedidos ML/Ensemble OK.
- [x] Lote 4 ui/tabs/configuracion.py: cada archivo del uploader se procesa una vez por file_id (_archivo_ya_procesado/_marcar_procesado) en inventario, historico, ofertas, protegidos, ATC y temperatura. Verificado con AppTest y uploader simulado, antes/despues: 1 carga + 5 clics -> antes se perdian 6 de 24 snapshots de auditoria, reescribia inventario.parquet y el Excel pisaba la edicion manual de protegidos (99->5); ahora se pierde solo 1 (el snapshot legitimo de la carga), no reescribe y conserva 99. Cargas de Auditoria (facturas, fisico) no escriben: sin cambio.
- [x] Lote 5 ui/tabs/pedidos.py: el pedido guarda sus parametros (pedido_params: lab, modo, meses, presupuesto, motor, nivel) y resultados/confirmacion usan esos; si cambian los widgets, aviso y confirmar deshabilitado; una confirmacion por pedido (pedido_confirmado, boton deshabilitado + guarda en servidor). Verificado AppTest antes/despues: 3 clics en Confirmar (pedido 'Todos', 7 labs) -> antes +21 registros y +3 snapshots KPI; ahora +7 y +1. Cambiar laboratorio tras generar -> aviso y boton deshabilitado; volver al lab generado y confirmar -> registra 1 con el lab correcto. Nota: AppTest no respeta disabled, por eso la guarda en servidor.
- [x] Lote 6 core/business.py: generar_pedido_presupuesto recalcula el tramo al recortar (max_unidades_en_presupuesto evalua cada tramo: mayor cantidad cuyo coste con el descuento de esa cantidad cabe; coste no monotono). Verificado Farmacia_Test presupuestos 150-3000: antes 3 de 7 con descuento indebido, ahora 0; coste siempre <= presupuesto. Casos borde OK. AppTest modo presupuesto 500 EUR: 498,91, sin excepciones.

- [x] Lote 7 (a) core/business.calcular_sobrestock(excluir_cns): donut Auditoria/BI y snapshot sin stock zombie; grafico de concentracion sin zombie ni UVI. Farmacia_Test: sobrestock 3.676,89 -> 1.751,19 (0 solapes con zombie 1.925,70); BI ya no pasa 0. (b) calcular_riesgo_caducidad sin Fecha_Caducidad valida -> None ("Sin datos"), sin mock; dayfirst=True. (c) network.registrar_pedido_confirmado: dias = (stock actual + pedido) / venta diaria de la propia linea (demanda ML si el pedido es ML); pedidos 'Todos' toma el minimo de los registros y registra lineas sin laboratorio como 'General'. Antes 14-41 dias, ahora ~66 para 2 meses de cobertura. (d) ML y Ensemble respetan el modo presupuesto (_pedido_desde_demanda -> generar_pedido_presupuesto con z del nivel de servicio): 500 EUR -> 499,01 / 499,82. AppTest sin excepciones; banco ML 7/7.

## Estado tras Fase 2 (lotes aprobados completos) — pendientes del plan original, sin aprobar
- Medio: robustez que corta pestanas (hoja Excel invalida, lineas sin laboratorio fuera del Excel, PDF unicode, CSV facturas sin try); donut con zombies duplicados; caducidad 'mock'; Fecha_Caducidad sin dayfirst; PDF 'ahorro acumulado' desde snapshots y escrito a disco en cada rerun; dias de cobertura sin stock; ML ignora modo presupuesto; guardar_dataframe_farmacia traga errores; validacion de columnas/tipos al cargar.
- Bajo: crear_farmacia sin sanear, stock negativo, KeyErrors defensivos, B905.
- Torre de control: no tocar (decision usuario). Ofertas por laboratorio: sin respuesta.
